    ...
    <div className="p-4 w-full bg-white">
        ...
        <div className="mb-3 flex items-center gap-2">
          <div className="relative">
            ...
            <button
              ...
              type="button"
            >
              <X className="w-3 h-3 bg-gray-300" />
            </button>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSendMessage} className="flex items-center gap-2">
        <div className="flex-1 flex gap-2">
          <input
            type="text"
            className="w-full input input-bordered rounded-lg input-sm sm:input-md bg-white text-black"
          ...
          <button
            type="button"
            className={`hidden sm:flex btn btn-circle bg-gray-200
                     ${imagePreview ? "text-emerald-500" : "text-gray-400"}`}
            onClick={() => fileInputRef.current?.click()}
          >
            <Image size={20} className="bg-gray-200"/>
          </button>
        </div>
        <button
          type="submit"
